...
export default function FinanceCRM() {
  ...
  const heroCopy = {
    overview: {
      ...
      title: "Visao Geral",
      subtitle: "Resumo executivo com filtros rapidos e graficos.",
    },
    pendencias: {
      eyebrow: "Gestao de risco",
      title: "Pendencias e alertas",
      subtitle: "Atrasos, aging e proximos pagamentos destrinchados.",
    },
    historico: {
      eyebrow: "Registros",
      title: "Historico completo",
      subtitle: "Tabela detalhada com exportacao e filtros.",
    },
    clientes: {
      eyebrow: "Carteira",
      title: "Clientes e concentracao",
      subtitle: "Ranking, participacao e status por cliente.",
    },
  };

  const currentHero = heroCopy[activePage] || heroCopy.overview;
  ...
  const heroTags = useMemo(() => {
    switch (activePage) {
      case "pendencias":
        return [
          ...
          `Proximos: ${proximosPagamentos.length}`,
          `Proj. 30d: ${BRL(proj30d)}`,
        ];
      case "historico":
        return [
          `Filtrados: ${filtered.length}`,
          `Valor: ${BRL(total)}`,
          `% Pago: ${pctPago.toFixed(1)}%`,
        ];
      case "clientes":
        return [
          `Top cliente: ${byCliente[0]?.cliente || "-"}`,
          `Top 5: ${portfolioShare.top5Share.toFixed(1)}%`,
          `Carteira: ${BRL(portfolioShare.totalValor)}`,
        ];
      default:
        return [
          `Registros: ${rows.length}`,
          `Clientes: ${clientes.length - 1}`,
          `Periodo: ${quickRange === "Todos" ? "Completo" : quickRange}`,
        ];
    }
  }, [
    activePage,
    ...
    quickRange,
    byCliente,
    portfolioShare.top5Share,
    portfolioShare.totalValor,
  ]);

  const PageHero = () => (
    <div className="card hero">
      <div>
        <div className="eyebrow">{currentHero.eyebrow}</div>
        <div className="page-title">{currentHero.title}</div>
        <p className="page-subtitle">{currentHero.subtitle}</p>
  ...
  const AlertsSummary = ({ showUpcoming = true }) => (
    <div className="alerts-row">
      {atrasados.length > 0 && (
        ...
        >
          <div className="alert-bar">
            <span className="alert-title">Pagamentos em atraso</span>
            <span
              className="alert-summary"
              title={`Clientes em atraso: ${atrasados.map((r) => r.cliente).join(", ")}`}
            >
              {atrasados.length} registro(s){" "}
              {atrasados.map((r) => r.cliente).slice(0, 2).join(", ")}
              {atrasados.length > 2 && ` e mais ${atrasados.length - 2} cliente(s)`}
            </span>
      ...
      {showUpcoming && proximosPagamentos.length > 0 && (
        ...
        >
          <div className="alert-bar">
            <span className="alert-title">Proximos Pagamentos (mes atual)</span>
            <span className="alert-summary">
              {proximosPagamentos.length} registro(s){" "}
              {proximosPagamentos.map((r) => r.cliente).slice(0, 2).join(", ")}
              {proximosPagamentos.length > 2 && ` e mais ${proximosPagamentos.length - 2} cliente(s)`}
            </span>
  ...
  const KpiGrid = () => (
    <div className="grid kpi-grid">
      ...
      <div className="card kpi-card">
        ...
        <div className="kpi-value">{byCliente[0]?.cliente || "-"}</div>
  ...
  const PendenciasKpis = () => {
    const ticketPend =
      atrasados.length === 0 ? 0 : totalPend / Math.max(atrasados.length, 1);
    return (
      <div className="grid kpi-grid">
        <div className="card kpi-card warning">
          <div className="kpi-title">Pendencias (valor)</div>
          <div className="kpi-value">{BRL(totalPend)}</div>
          <div className="kpi-sub">Alertas: {atrasados.length}</div>
        </div>
        <div className="card kpi-card info">
          <div className="kpi-title">Proximos 30 dias</div>
          <div className="kpi-value">{BRL(proj30d)}</div>
          <div className="kpi-sub">
            Registros: {proximosPagamentos.length}
          </div>
        </div>
        <div className="card kpi-card muted">
          <div className="kpi-title">Aging medio</div>
          <div className="kpi-value">
            {avgAging ? `${avgAging.toFixed(0)} dias` : "N/A"}
          </div>
          <div className="kpi-sub">Ticket pendente: {BRL(ticketPend)}</div>
        </div>
        <div className="card kpi-card accent">
          <div className="kpi-title">Carteira em risco</div>
          <div className="kpi-value">
            {BRL(agingBuckets.totalPendBucket || 0)}
          </div>
          <div className="kpi-sub">% pago atual: {pctPago.toFixed(1)}%</div>
        </div>
      </div>
    );
  };
        ...
        <table>
          ...
          <tbody>
            {filtered.slice(0, limit).map((row, index) => (
              <tr key={index}>
                {columnMap.map((col, i) => {
                  const rawValue = row[col.key] || row[col.key.replace(/\s/g, "_")];
  ...
  const ClientesKpis = () => {
    const clientesAtivos = clientes.length - 1;
    const ticketMedio = filtered.length ? total / filtered.length : 0;
    return (
      <div className="grid kpi-grid">
        <div className="card kpi-card">
          <div className="kpi-title">Top cliente</div>
          <div className="kpi-value">{byCliente[0]?.cliente || "-"}</div>
          <div className="kpi-sub">Valor: {BRL(byCliente[0]?.valor || 0)}</div>
        </div>
        <div className="card kpi-card">
          <div className="kpi-title">Concentracao top 5</div>
          <div className="kpi-value">{portfolioShare.top5Share.toFixed(1)}%</div>
          <div className="kpi-sub">Carteira: {BRL(portfolioShare.totalValor)}</div>
        </div>
        <div className="card kpi-card">
          <div className="kpi-title">Clientes ativos</div>
          <div className="kpi-value">{clientesAtivos}</div>
          <div className="kpi-sub">Ticket medio: {BRL(ticketMedio)}</div>
        </div>
        <div className="card kpi-card">
          <div className="kpi-title">Saude da carteira</div>
          <div className="kpi-value">{pctPago.toFixed(1)}% pago</div>
          <div className="kpi-sub">Pendencias: {BRL(totalPend)}</div>
        </div>
      </div>
    );
  };
            ...
            {portfolioShare.ranked.map((c, i) => (
              <tr key={i}>
                ...
                <td>
                  <button
                    className="chip"
                    onClick={() => setCliente(c.cliente || "Todos")}
                  >
                    Filtrar
                  </button>
                </td>
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      default:
        return (
          <>
            <AlertsSummary showUpcoming={false} />
            <KpiGrid />
            <OverviewCharts />
            <TableCard />
          </>
        ...
        <div className="container header-inner">
          <div className="header-left">
            ...
            <div className="brand-stack">
              ...
              <span className="header-subtitle">Finance CRM</span>
            </div>
          </div>
